...
async def on_guild_join(self, guild):
    #write to the serverlangs.json file
...
async def say(ctx, *, arg):
    if ctx.author.id == 726512497649254481 or ctx.author.id == 783417515236917260:
        print(f"User {str(ctx.author.id)} blocked from using this command")
        return
    else:
...
async def info(ctx):
    ...
    embed.set_thumbnail(url=(thumbnail))
    message = await ctx.reply(embed=embed, mention_author=True)
    delta_uptime = datetime.datetime.now() - startTime
    await ctx.send(delta_uptime)
...
async def iplookup(ctx, arg):
    ip = requests.get('http://api.ipstack.com/' + (str(arg)) + '?access_key=' + (ipapi_key))
...
async def math(ctx, *args):
    if args[0] == '-area':
        if args[1] == '-rectangle':
            ...
            embed=discord.Embed(title='Area commands', description='`-rectangle` \n `-triangle` \n `-circle` \n `-rhombus`', color=(color))
            embed.set_footer(text=(description))
            embed.set_thumbnail(url=(thumbnail))
            message = await ctx.reply(embed=embed, mention_author=True)
...
async def on_command_error(ctx, error):
    ...
    if str(error)[-9:] == 'not found': #'<:__:842869170990874646>'
        emojis = ['<:N:842869093275271199>', '<:O:842869124178509854>', '<:T:842869147150450748>', '<:F:842869215283511337>', '<:O2:842869245070409771>', '<:U:842869292156583966>', '<:N2:842869322833199164>', '<:D:842869349517361223>']
        for emoji in emojis:
